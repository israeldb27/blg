...
public class ImovelDaoImpl extends GenericDAOImpl<Imovel, Long>   implements ImovelDao {
	...
	// melhorar ainda a query deste método
	@Override	
	public List<Imovel> findSugestoesImoveisParaClientes(Long idUsuario, int regra){
		
		Criteria crit = session().createCriteria(Imovel.class);			
		if ( regra == 1){ // recuperar pelo menos uma preferencia completa localidade do cliente		
			Preferencialocalidade pref = preferencialocalidadeDao.findPreferenciaRandomByIdUsuario(idUsuario);	
			if ( pref != null ){
				crit.createCriteria("usuario").add(Restrictions.eq("id", idUsuario)); 
			    crit.add(Restrictions.eq("idEstado", pref.getIdEstado()));
			    crit.add(Restrictions.eq("acao", pref.getAcao()));		    
			    crit.add(Restrictions.eq("tipoImovel", pref.getTipoImovel()));	
			    if ( pref.getIdCidade() > 0 )
			    	crit.add(Restrictions.eq("idCidade", pref.getIdCidade()));
			}					
		}
		else if ( regra == 2){
			Imovelvisualizado imovelVisitado = imovelvisitadoDao.findImovelVisitadoRandomByIdUsuario(idUsuario);
			if ( imovelVisitado != null)
				crit.add(Restrictions.eq("id", imovelVisitado.getImovel().getId()));
		}
		else if ( regra == 3){
			Preferencialocalidade pref = preferencialocalidadeDao.findPreferenciaRandomByIdUsuario(idUsuario);
			if ( pref != null ){
			    ...
			    if ( pref.getIdBairro() > 0 )
			    	crit.add(Restrictions.eq("idBairro", pref.getIdBairro()));
	...
	// melhorar ainda a query deste método
	@Override
	public List<Imovel> findSugestoesImoveisParaCorretoresImobiliarias(Long idUsuario, int regra){
		Criteria crit = session().createCriteria(Imovel.class);			
		Criteria critUsuario = crit.createCriteria("usuario");
		if ( regra == 1 ){ // imoveis para intermediacao
			critUsuario.add(Restrictions.ne("id", idUsuario));
			crit.add(Restrictions.eq("autorizacaoOutroUsuario", "S"));
			critUsuario.add(Restrictions.eq("perfil", PerfilUsuarioOpcaoEnum.PADRAO.getRotulo()));			
		}
		else if ( regra == 2 ) { // imoveis para parceria -- corretor 
			critUsuario.add(Restrictions.ne("id", idUsuario)); 
			crit.add(Restrictions.eq("autorizacaoOutroUsuario", "S"));
			critUsuario.add(Restrictions.eq("perfil", PerfilUsuarioOpcaoEnum.CORRETOR.getRotulo()));
		}
		else if ( regra == 3 ) { // imoveis para parceria -- imobiliaria 
			critUsuario.add(Restrictions.ne("id", idUsuario)); 
			crit.add(Restrictions.eq("autorizacaoOutroUsuario", "S"));
			critUsuario.add(Restrictions.eq("perfil", PerfilUsuarioOpcaoEnum.IMOBILIARIA.getRotulo()));			
		}
		else if ( regra == 4 ) {
			Imovelvisualizado imovelVisitado = imovelvisitadoDao.findImovelVisitadoRandomByIdUsuario(idUsuario);
			if ( imovelVisitado != null )
			...
			Contato contato = contatoDao.findContatoRandomByIdUsuarioByStatus(idUsuario, ContatoStatusEnum.OK.getRotulo());
			if ( contato != null ){
				if ( contato.getUsuarioConvidado().getId().longValue() == idUsuario.longValue() ){					
					critUsuario.add(Restrictions.ne("id", contato.getUsuarioHost().getId()));
				}
				else if ( contato.getUsuarioHost().getId().longValue() == idUsuario.longValue() ){
					critUsuario.add(Restrictions.ne("id", contato.getUsuarioConvidado().getId()));					
				}
			}			
		}
