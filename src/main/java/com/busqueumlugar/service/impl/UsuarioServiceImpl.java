					...
					if ( regra == 1 ){
						...
						if (! CollectionUtils.isEmpty(listaIds)){ // para o caso de o usuario ainda não ter nenhum contato ou usuarios seguindo
							...
							while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4)){
								...
								if ( ( regraSel >= 1 ) && ( regraSel <= 5 )) { // Exibir imóveis de seus contatos e usuários que esteja seguindo
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisCompartilhadosIdsUsuarios(session, "N");
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
								else if ( ( regraSel >= 6 ) && ( regraSel <= 8 )) { // Exibir imóveis de seus contatos e /ou usuario que esteja seguindo que deseje ser intermediado ou queira parceria
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisCompartilhadosIdsUsuarios(session, "S");
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
								else if ( ( regraSel >= 9 ) && ( regraSel <= 11 )) { // Exibir Notas de seus contatos e usuários que esteja seguindo
								...
								else if ( ( regraSel >= 12 ) && ( regraSel <= 14 )) { // Exibir uma preferencia de imóvel de algum contato e/ou usuário que esteja seguindo
								...
								while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4)  ){
									...
									if (( regraSel >= 1 ) && ( regraSel <= 5 )) { // Exibir aleatoriamente algum imóvel que deseje ser intermediado ou queira alguma parceria
										List<Imovel> lista = this.regraTimeLineRecuperarImoveisCompartilhadosSemContato(user, session, "S");
										if (! CollectionUtils.isEmpty(lista))
											listaFinal.addAll(lista);
									}
									else if (( regraSel >= 6 ) && ( regraSel <= 8 )) { // Exibir aleatoriamente algum usuário e sua respectiva preferencia de imóvel
					...
					if ( regra == 1 ){ // poderá nao ser exibido um novo anuncio de imóvel consecutivo 
						...
						if ( CollectionUtils.isEmpty(listaIds)){ // para o caso de o usuario ainda não ter nenhum contato ou usuarios seguindo
							...
							while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4) || (isNotaExiste) ){
								...
								else if ( ( regraSel >= 6 ) && ( regraSel <= 8 )) { // Recuperar Imóveis de acordo com a Preferencia de Imóveis
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisPreferencia(user, session);
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
