...
public class UsuarioServiceImpl implements UsuarioService{
	...
	public String validarCadastroUsuario(UsuarioForm frm) {	
		 String msg = "";
	        ...
	        if ( frm != null){            
	            ...
	            // validando se existe jï¿½ um usuario com o cpf ou cnpj informado
	            if ( msg.equals("")){                
	            ...
	            // para o caso corretor verficar se jï¿½ existem alguem com o creci informado
	            if ( msg.equals("")){
	...
	public String validarEdicaoUsuario(UsuarioForm frm) {	
        ...
        if ( frm != null){            
            /*if ( ! frm.getPassword().equals(frm.getConfirmaPassword()))
                msg = "Os campos password e confirma password nï¿½o conferem";
            else
	...
	public String validarMudancaSenha(UsuarioForm frm) {	
        ...
        // acrescentar depois outras regras para formaï¿½ï¿½o de senha         
        if ( msg.equals("")){
	...
	public void tratarTelaInicial(UsuarioForm user, HttpSession session, ModelMap map) {
		/*
		* Obs.: A sugestÃ£o de imoveis e/ou usuÃ¡rios devem ser no mÃ¡ximo de 4 registros (por enquanto).
		*
		* Obs.: Se , por exemplo, no perfil Cliente se recuperar apenas 1 imÃ³vel de acordo com a preferencia de imÃ³veis do usuÃ¡rio entÃ£o
		*       completar a lista com outros 3 imÃ³veis sugerindo-o aleatoriamente 
		*
		* Obs.: Estes imÃ³veis sugeridos nÃ£o podem pertencer ao usuÃ¡rio. E os usuÃ¡rios sugeridos nÃ£o podem ser contato do usuÃ¡rio.
		*/
	...
	public String validarAcessoUsuario(UsuarioForm user) {
		...
		if (user.getDataValidadeAcesso().compareTo(new Date()) < 0)
			msg = "N";
			//msg = "Usuï¿½rio possui data de validade de acesso expirada";
	...
	public List<Usuario> sugerirUsuarios(UsuarioForm user){
		/*  
		 *   Obs.: Clientes --> Sugerir corretores e/ou imobiliarias para auxiliar na intermediaÃ§Ã£o de um imÃ³vel seu e sugerir 
		 ...
		 *	
		 *	 Obs.: Corretores e ImobiliÃ¡rias -->  sugerir usuarios clientes que podem ter interesse em imÃ³veis seus de acordo com a preferencia 
		 *										  de imÃ³veis e sugerir outros usuÃ¡rios aleatoriamente que nÃ£o sejam seus contatos
	...
	public void prepararParaCarregarTimeLine(UsuarioForm user, HttpSession session, ModelMap map) {
		...
		/**
		 * Utilizar uma Hashtable para também carregar os Ids dos usuarios e para cada entrada na Hashtable carrega 
		 * a respectiva "Index Posicao" que será usado para carregar um imóvel do usuário e minimizar a repetição na exibição de imóveis 
		 *   
		 */
		...
		// carregar a quantidade de imóveis anuncios do dia
	...
	public List<String> carregar(UsuarioForm user, HttpSession session){
			// 	independente do perfil, carregar a TimeLine de acordo com a regra selecionada
				...
				/**		  
				 * Por default, exibir sempre no primeiro registro um Imóvel Anúncio (se houver). Exibindo primeiramente o anuncio com maior relevância e/ou mais recente
				 */
				
				/***
				 * Talvez separar cada regra por método para reunir o Imóvel ou lista de Imóveis para depois montar a TimeLine	
				 */
				...
				if (! user.getPerfil().equals(PerfilUsuarioOpcaoEnum.PADRAO.getRotulo())){ // Inicio Timeline para Corretores e Imobiliaris 
					...
					if ( regra == 0 ){ // usuario acabou de se logar no sistema então primeiro registro na timeline será um Anúncio (se houver)	-- Teoricamente vai ser um anuncio de grande Importancia			
					...
					if ( regra == 1 ){
						...
						if (! CollectionUtils.isEmpty(listaIds)){ // para o caso de o usuario ainda não ter nenhum contato ou usuarios seguindo
							
							while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4)){
								
								regraSel = r.nextInt(18) + 1;								
								
								if ( ( regraSel == 1 ) || ( regraSel == 2 ) || ( regraSel == 3 ) || ( regraSel == 4 ) || (regraSel == 5)) { // Exibir imóveis de seus contatos e usuários que esteja seguindo
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisCompartilhadosIdsUsuarios(session, "N");
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
								else if ( ( regraSel == 6 ) || ( regraSel == 7 ) || ( regraSel == 8)) { // Exibir imóveis de seus contatos e /ou usuario que esteja seguindo que deseje ser intermediado ou queira parceria
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisCompartilhadosIdsUsuarios(session, "S");
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
								else if ( ( regraSel == 9 ) || ( regraSel == 10 ) || ( regraSel == 11)) { // Exibir Notas de seus contatos e usuários que esteja seguindo
								...
								else if ( ( regraSel == 12 ) || ( regraSel == 13 ) || ( regraSel == 14 ) ) { // Exibir uma preferencia de imóvel de algum contato e/ou usuário que esteja seguindo
								...
								else if ( ( regraSel == 15 ) || ( regraSel == 16 )) { // Exibir uma anuncio imóvel  
									Imovel imovel = this.regraTimeLineRecuperarImovelAnuncio(session);
									if ( imovel != null)
										listaFinal.add(imovel);									
								}
								else if ( ( regraSel == 17 ) || ( regraSel == 18 )) { // Exibir aleatoriamente uma preferencia de imóvel de algum usuário da plataforma  
								...
								else if ( ( regraSel == 19) ) { // Exibir aleatoriamente algum imóvel da plataforma  
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisAleatoriamente(listaIds, session);
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}	
								else if ( ( regraSel == 20 )  ) { // Sugerir algum usuário para formalizar contato ou para seguir
								...
								while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4)  ){
									...
									if ( ( regraSel == 1 ) || ( regraSel == 2 ) || ( regraSel == 3 ) || ( regraSel == 4 ) || (regraSel == 5)) { // Exibir aleatoriamente algum imóvel que deseje ser intermediado ou queira alguma parceria
										List<Imovel> lista = this.regraTimeLineRecuperarImoveisCompartilhadosSemContato(user, session, "S");
										if (! CollectionUtils.isEmpty(lista))
											listaFinal.addAll(lista);
									}
									else if ( ( regraSel == 6 ) || ( regraSel == 7 ) || ( regraSel == 8 ) ) { // Exibir aleatoriamente algum usuário e sua respectiva preferencia de imóvel
									...
									else if ( ( regraSel == 9 ) || ( regraSel == 10 ) ) { // Sugerir algum usuário para formalizar contato ou para seguir
									...
									else if ( ( regraSel == 11 ) || ( regraSel == 12 ) ) { // Exibir anúncio de imóvel
										Imovel imovel = this.regraTimeLineRecuperarImovelAnuncio(session);
										if ( imovel != null )
											listaFinal.add(imovel);								
									}					
					...
					if ( regra == 1 ){ // poderá nao ser exibido um novo anuncio de imóvel consecutivo 
						...
						if ( CollectionUtils.isEmpty(listaIds)){ // para o caso de o usuario ainda não ter nenhum contato ou usuarios seguindo
							...
							while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4)){
								...
								if ( ( regraSel == 1 ) || ( regraSel == 2 ) || ( regraSel == 3 ) || ( regraSel == 4 ) || (regraSel == 5)) { // Recuperar Imóveis de acordo com a Preferencia de Imóveis
								...
								else if ( ( regraSel == 8 ) || ( regraSel == 9 )) { // Exibir um anucio imóvel
									Imovel imovel = this.regraTimeLineRecuperarImovelAnuncio(session);
									if ( imovel != null)
										listaFinal.add(imovel);
								}
								else if ( ( regraSel == 10 ) || ( regraSel == 11 )) { // Exibir uma indicação de usuário  
									usuarioTimeline = dao.findUsuario(124l);
									if ( usuarioTimeline != null){
										isUsuarioTimeLine = true;
										break;
							...
							while ( (AppUtil.recuperarQuantidadeLista(listaFinal) < 4) || (isNotaExiste) ){
								...
								if ( ( regraSel == 1 ) || ( regraSel == 2 ) || ( regraSel == 3 ) || ( regraSel == 4 ) || (regraSel == 5)) { // exibir algum imovel de um contato ou de um usuário que esteja seguindo
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisIdsUsuarios(session);
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
								else if ( ( regraSel == 6 ) || ( regraSel == 7 ) || ( regraSel == 8 ) ) { // Recuperar Imóveis de acordo com a Preferencia de Imóveis
									List<Imovel> lista = this.regraTimeLineRecuperarImoveisPreferencia(user, session);
									if (! CollectionUtils.isEmpty(lista))
										listaFinal.addAll(lista);
								}
								else if ( ( regraSel == 9 ) || ( regraSel == 10 ) ) { // Exibir Imóvel Anúncio
									Imovel imovel = this.regraTimeLineRecuperarImovelAnuncio(session);
									if ( imovel != null )
										listaFinal.add(imovel);
								}						
	...
	public List<Imovel> regraTimeLineRecuperarImoveisIdsUsuarios(HttpSession session){
		...
		if ( ! CollectionUtils.isEmpty(listaIds)) {
			...
			List<Imovel> listaImovel = imovelService.recuperarImovelPorIdsUsuarioPorPosicao(listaIds, index); // o parametro informado é index para o setFirstResult no Hibernate 
	...
	public List<Imovel> regraTimeLineRecuperarImoveisCompartilhadosIdsUsuarios(HttpSession session, String aceitaCompartilhado){ // o parametro aceitaCompartilhado filtra se o Imovel é compartilhado ou nao
		List<Long> listaIds = (List<Long>)session.getAttribute(TimelineService.LISTA_IDS_USUARIOS); 
		if ( ! CollectionUtils.isEmpty(listaIds)) {			
			if ( aceitaCompartilhado.equals("S")){
				int index = (int)session.getAttribute(TimelineService.ULTIMO_INDEX_IDS_USUARIO_COMPARTILHADO);					
				List<Imovel> listaImovel = imovelService.recuperarImovelPorIdsUsuarioPorPosicaoPorAceitaCompartilhado(listaIds, index, aceitaCompartilhado); // o parametro informado é index para o setFirstResult no Hibernate 
				int quant = AppUtil.recuperarQuantidadeLista(listaImovel);
	...
	public List<Imovel> regraTimeLineRecuperarImoveisCompartilhadosSemContato(UsuarioForm user, HttpSession session, String aceitaCompartilhado){ // o parametro aceitaCompartilhado filtra se o Imovel é compartilhado ou nao		 
				
			if ( aceitaCompartilhado.equals("S")){
				int index = (int)session.getAttribute(TimelineService.ULTIMO_INDEX_IDS_USUARIO_COMPARTILHADO);					
				List<Imovel> listaImovel = imovelService.recuperarImovelPorIdsUsuarioPorPosicaoPorAceitaCompartilhado(user.getId(), index, aceitaCompartilhado); // o parametro informado é index para o setFirstResult no Hibernate 
				int quant = AppUtil.recuperarQuantidadeLista(listaImovel);
				session.setAttribute(TimelineService.ULTIMO_INDEX_IDS_USUARIO_COMPARTILHADO, index + quant);
				return listaImovel;
			}
				...
				int index = (int)session.getAttribute(TimelineService.ULTIMO_INDEX_IDS_USUARIO);					
				List<Imovel> listaImovel = imovelService.recuperarImovelPorIdsUsuarioPorPosicao(user.getId(), index); // o parametro informado é index para o setFirstResult no Hibernate 
				int quant = AppUtil.recuperarQuantidadeLista(listaImovel);
				session.setAttribute(TimelineService.ULTIMO_INDEX_IDS_USUARIO, index + quant);
				return listaImovel;
			}
	...
	public String carregarTimeLineNota(Nota nota, UsuarioForm user){
		 ...
		 buf.append("    		<img class='timeline-badge-userpic' src='" + context.getContextPath() + nota.getUsuario().getImagemArquivo() + "' style='width: 72px; height: 72px; ' />  ");
		 buf.append(" 			</a>");
		 buf.append("		</div>  ");
	     ...
	     if ( nota.getAcao().equals(NotaAcaoEnum.PREFERENCIA.getRotulo()) || nota.getAcao().equals(NotaAcaoEnum.USUARIO.getRotulo()) ) {	 
			 ...
			 buf.append("                       <img src='" + context.getContextPath() + nota.getImovel().getImagemArquivo()  +  "' style='width: 60px; height: 60px; ' alt='admin'/> ");
		 }		 						                                                                                           	                              
		 buf.append("                     </span>  ");
	...
	public String carregarTimeLineImovel(Imovel imovel) {
		   ...
		   buf.append("    			<img class='timeline-badge-userpic' src='" + context.getContextPath() + imovel.getUsuario().getImagemArquivo() + "' style='width: 72px; height: 72px; ' />  ");
		   buf.append(" 		</a>");
		   buf.append("   </div> ");
		   buf.append("   <div class='timeline-body'> ");
		   buf.append("    	<div class='timeline-body-arrow'>  "); 
	...
	public String carregarTimeLineUsuario(Usuario usuario, UsuarioForm user) {
	    ...
	    buf.append("				  		<img class='timeline-badge-userpic' src='"+ context.getContextPath() + usuario.getImagemArquivo() +"' style='width: 90px; height: 90px; ' /> &nbsp; ");
	    buf.append("				  </a> ");                                                          
	    buf.append("      	  </div> ");
	    buf.append("      <div class='timeline-body'> ");    
	...
	public String carregarTimeLinePreferenciaLocalidade(Preferencialocalidade pref) {
	    ...
	    buf.append("				  		<img class='timeline-badge-userpic' src='"+ context.getContextPath() + pref.getUsuario().getImagemArquivo() +"' style='width: 90px; height: 90px; ' /> &nbsp; ");
	    buf.append("				  </a> ");                                                          
	    buf.append("      	  </div> ");
	    buf.append("      <div class='timeline-body'> ");    
	    buf.append("		  <div class='timeline-body-arrow'> </div> ");
