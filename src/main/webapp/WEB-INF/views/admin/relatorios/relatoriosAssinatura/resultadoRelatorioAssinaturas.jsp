                                                    	...
                                                    	<div class="table-responsive" style="margin-top: -1px;">
					                                        <table class="table table-default">
					                                            ...
					                                            <tbody>
																	...
																	<c:if test="${administracaoForm.tipoRelatorio == 'assinaturasVolFinanceiro'}">
																		<c:forEach var="imovel" items="${administracaoForm.listaRelatorioQuantAssinatura}" varStatus="item">
																			<tr class="border-primary">	
																				<td><c:if test="${assinatura == 'assinaturaPadrao'}">										
																						<spring:message code="lbl.status.assinatura.cliente"/>	                   							
																					</c:if>
																					<c:if test="${assinatura == 'assinaturaCorretor'}">
																						<spring:message code="lbl.status.assinatura.corretor"/>
																					</c:if>
																					<c:if test="${assinatura == 'assinaturaImobiliaria'}">
																						<spring:message code="lbl.status.assinatura.imobiliaria"/>	                   							
																					</c:if>
																				</td>						                                                					                                                					                                               
																				<td><fmt:formatNumber value="${assinatura.valorFinanceiro}" pattern="#,##0.00;-0"/> </td>
																			</tr>
																		</c:forEach>
																	</c:if>                                    
																	<c:if test="${administracaoForm.tipoRelatorio == 'buscaAssinaturasUsuario'}">
																		<c:forEach var="assinatura" items="${administracaoForm.listaRelatorioQuantAssinatura}" varStatus="item">
																			<tr class="border-primary">	
																				<td><a href="${urlAdmin}/detalhesUsuario/${usuarioBusca.id}" > ${assinatura.nomeUsuario}</a>  </td> 
																				<td><c:if test="${assinatura == 'assinaturaPadrao'}">										
																						<spring:message code="lbl.status.assinatura.cliente"/>	                   							
																					</c:if>
																					<c:if test="${assinatura == 'assinaturaCorretor'}">
